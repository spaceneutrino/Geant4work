...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  //
  G4double cryst_r = 1*cm, cryst_x = 54*cm;
  G4int nb_cryst = 100;
  G4int nbn_cryst = 100;
  G4int num_rings = 5;
  //
  ...
  //
  G4double ring_R1 = cryst_r/tandPhi;
  ...
  for (G4int i_ring = 1; i_ring < 9 ; i_ring++) {
  ring_r = ring_R2-2.0*cryst_r*i_ring;
//  nbn_cryst = twopi*ring_r/2.0/cryst_r;
  for (G4int icrys = 0; icrys < nbn_cryst ; icrys++) {
    G4double phi = icrys*twopi/nbn_cryst;
  ...
  //
  G4double OG = -0.5*(detector_dZ) + cryst_x; 
    new G4PVPlacement(0,                     //no rotation
